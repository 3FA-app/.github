...
- **Deterministic project registry:** generated by [`scripts/build-project-registry.py`](../scripts/build-project-registry.py) and published by [`project-registry-artifact.yml`](../.github/workflows/project-registry-artifact.yml)
...
The canonical `Repository role` field options are `governance`, `interfaces`, `clients`, `cli`, `backend`, `web`, `desktop`, `sync`, `infra`, `e2e`, `mcp`, `website`, `extension`, and `monorepo`. The current CLI repository is [`3FA-app/3FA-app-cli`](https://github.com/3FA-app/3FA-app-cli).

## Published registry contract

The registry workflow builds and uploads three public-safe files from `organization-routing.json`:

- `3fa-app-project-registry.json`;
- `3fa-app-project-registry.md`;
- `SHA256SUMS`.

The JSON and Markdown are deterministic, contain all 14 canonical repositories exactly once, and reciprocally identify the GitHub organization, GitHub Project #1, Linear project, Linear team, and Slack channel. The artifact is execution evidence only; it does not grant or imply GitHub Projects administration access.
